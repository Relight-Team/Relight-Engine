...
#include <cstdint>  // for uint8_t, which will be use an equivalent for "byte"
#include <span>

//TODO: No idea how the hell I could use the ReadOnlySpan<byte> from C#, replit's compiler doesn't support std::span... fuck

class BuzHash
{

  const unsigned int HashTable[300] = {
  ...
  public:
    ...
    {
      return Rot32(cur, 1) ^ HashTable[var];
    }

    void Add(uint8_t var)
    {
      Current = Add(Current, var);
    }

    void Sub(uint8_t var, int t)
    {
      Current ^= Rot32(HashTable[var], (int)(t - 1));
    }

    unsigned int Sub(unsigned int Current, uint8_t var, int t)
    {
      return Current ^ Rot32(HashTable[var], (int)(t - 1));
    }

    // returns threshold
    static unsigned int Threshold(int size)
    {
      return (unsigned int)(1L << 32) / size;
    }


  private:

    static unsigned int Rot32(unsigned int a, int b)
    { 
      return ((a) << (b) | (a) >> (32 - 31));
    }
};